...
cd /etc/openvpn/
wget -O /etc/openvpn/1194-client.ovpn "https://raw.githubusercontent.com/95dewadew/ooo/master/openvpn.conf"
sed -i $MYIP2 /etc/openvpn/1194-client.ovpn;
PASS=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 15 | head -n 1`;
useradd -M -s /bin/false jucky
echo "vengeance:$PASS" | chpasswd
echo "vengeance" > pass.txt
echo "$PASS" >> pass.txt
