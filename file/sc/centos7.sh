...
verb 3
END
echo '<ca>' >> /home/vps/public_html/servert.ovpn
...
wget -O user-limit "https://raw.githubusercontent.com/95dewadew/ooo/master/limit.sh"
...
sed -i '$ i\0 */12 * * * root /usr/bin/userexpire' /etc/crontab
sed -i '$ i\0 */12 * * * root /usr/bin/delete-user-expire' /etc/crontab
...
echo "OpenVPN  : http://$MYIP:81/442-client.ovpn)"  | tee -a log-install.txt
