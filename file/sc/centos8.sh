...
cd /etc/openvpn/
tar xf openvpn.tar
wget -O /etc/openvpn/server.conf "https://jagoanssh.com/file/sc/openvpn/tcp32.conf"
if [ "$OS" == "x86_64" ]; then
  wget -O /etc/openvpn/server.conf "https://jagoanssh.com/file/sc/openvpn/tcp64.conf"
fi
...
mkdir -p /home/vps/public_html
cat > /home/vps/public_html/server.ovpn <<-END
...
verb 3
END
echo '<ca>' >> /home/vps/public_html/server.ovpn
cat /etc/openvpn/keys/ca.crt >> /home/vps/public_html/server.ovpn
echo '</ca>' >> /home/vps/public_html/server.ovpn

cp client.tar /home/vps/public_html/
cp server.ovpn /home/vps/public_html/
...
sed -i '/Port 143/a Port 777' /etc/ssh/sshd_config
sed -i '/Port 777/a Port 80' /etc/ssh/sshd_config
sed -i '/Port 80/a Port 110' /etc/ssh/sshd_config
sed -i '/Port 110/a Banner /etc/banner.net' /etc/ssh/sshd_config
...
echo "OPTIONS=\"-b /etc/banner.net -p 2222\"" > /etc/sysconfig/dropbear
...
# install webmin
cd
wget https://raw.githubusercontent.com/95dewadew/ooo/master/app/webmin-1.710-1.noarch.rpm
...
wget -O user-limit "https://raw.githubusercontent.com/95dewadew/ooo/master/app/user-limit7"
...
sed -i '$ i\*/5 * * * * root /usr/sbin/vnstat.cron' /etc/crontab
sed -i '$ i\0 */6 * * * root /usr/bin/userexpire' /etc/crontab
sed -i '$ i\0 */6 * * * root /usr/bin/delete-user-expire' /etc/crontab
...
#seting sslh
cd
wget -O /etc/sslh.cfg "https://jagoanssh.com/file/sc/sslh.cfg"
...
echo " #                                                     # "  | tee -a log-install.txt
echo " #              CENTOS 7 32 BIT & 64 BIT               # "  | tee -a log-install.txt
echo " #=====================================================# "  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
...
echo "Dropbear : 80, 777, 110"  | tee -a log-install.txt
echo "Squid    : 8080, 3128"  | tee -a log-install.txt
echo "OpenVPN  : http://$MYIP:81/server.ovpn)"  | tee -a log-install.txt
