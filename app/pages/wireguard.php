...
switch($_GET['filter']) {
  case 'one-month':
    ...
    $dimas = "`type`='one-month' $filter OR `type`='wireguard' $filter";
  break;
  case 'extra':
    $dimas = "`type`='extra'";
  break;
  default:
    $filter = '';
    $dimas = "`type`='vpnssh' $filter OR `type`='wireguard' $filter";
  break;
}
else:
  $filter = '';
...
?>
<div id="where" style="display:none;">wireguard vpn</div>
<div id="ndexi" style="display:none;">wireguard vpn</div>
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-8 col-md-offset-2 section-heading mb50 text-center">
            <h1>FREE WIREGUARD VPN SERVERS</h1>
            <h2>Free Wireguard VPN 7 Days</h2>
           <p class="lead">Select Wireguard Server Location According to Your Needs</p>
          <p class="lead">Wireguard We have different Speeds for each country</p>
...
?>
    ...
    <p class="text-center">
    <span class="badge badge-success mb-3 p-3">Free Wireguard VPN</span>
...
<!-- Server -->
      ...
      <p align="center"><a class="btn btn-danger" href="<?php urlPrefix('create-wireguard/'.$row->id) ?>"><i class="fa fa-times" aria-hidden="true"></i> Create Now</a></p>
      <?php else: ?>
      <p align="center"><a class="btn btn-primary" href="<?php urlPrefix('create-wireguard/'.$row->id) ?>"><i class="fa fa-check" aria-hidden="true"></i> Create Now</a></p>
      <?php endif; ?>
    </div>
    </div>
  <!-- End Server -->
